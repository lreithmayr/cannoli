...
bool cannoli::Sphere::hit(const LightRay &ray, const float &t_min, const float &t_max, HitRecord &hit_record) {
  ...
  double a_2 = ray.getDirection().computeLengthSquared();
  double half_a1 = dot(dist_origin_center, ray.getDirection());
  double a_0 = dist_origin_center.computeLengthSquared() - (m_radius * m_radius);
  ...
  hit_record.t = root;
  hit_record.hit_point = ray.getPosition(root);
  hit_record.surface_normal = calculateSurfaceNormal(ray, hit_record.t, hit_record);

  return true;
}

cannoli::Vec3f cannoli::Sphere::calculateSurfaceNormal(const LightRay &ray, float t, HitRecord &hit_record) {
  Vec3f outward_normal = ((ray.getPosition(t) - m_center) * (1 / m_radius));
