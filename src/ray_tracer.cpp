...
void cannoli::RayTracer::SetupImage() {
    m_ppmImage.open(m_outFile, std::fstream::out | std::fstream::trunc);
    if (!m_ppmImage.is_open()) {
        fmt::print(stderr, "Output image could not be created\nError code: {}\n", strerror(errno));
        exit(-1);
    }
...
cannoli::Vec3f cannoli::RayTracer::GenerateDirection(const int pixel_x, const int pixel_y) {
    ...
    Vec3f
            dir = m_camera.GetViewportLLC() + m_camera.GetHorizontal() * u + m_camera.GetVertical() * v -
                  m_camera.GetOrigin();
    return dir;
}
...
cannoli::ColorRGB cannoli::RayTracer::PaintBackground(cannoli::LightRay &ray) {
    ...
    return 1.5 * ((1.0f - t) * ColorRGB(1.0, 1.0, 1.0) + t * ColorRGB(0.5, 0.7, 1.0));
}
